...
Because this feature relies on LeetCode page internals, it is **off by default**: enable
"Show dislike counts on comments" in the extension's toolbar popup. While the comment feature
is disabled, the page-world script stays completely dormant — it only ever acts when the
extension asks it to, which never happens with the feature off. Toggling takes effect
immediately, even on already-open LeetCode pages.
...
correctly across in-app (SPA) navigation without a page reload, and a comments test that
enables the opt-in comment counts through the popup and checks that editorial and discuss post
comments show them. CI (and the release gate) runs only the smoke test; the other tests run locally
via `./test.sh`.
